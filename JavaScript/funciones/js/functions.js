...
function findLongestWord( array_words ){
     ...
     for (var index = 0; index < array_words.length; index++) {
     	if (array_words[index].length > lengthWord) {
     		lengthWord = array_words[index].length;
     		longestWord = array_words[index].length;
     	}
     }
     return longestWord
}
...
array_lengths.push( findLongestWord( ["Otorrinolaringología", "Electroencefalografía", "Supercalifragilisticoespialidoso"] ) );
/*
...
function charFreq( string ) {
  ...
  for (var index = 0; index < separate.length; index++) {
    var key = separate[index];
    if (freqCounter[key] === undefined){
      freqCounter[key] = 1
    } else {
      freqCounter[key] += 1
    }
    ...
    for (var i = 0; i < array.length; i++) {
       if (isNaN(array[i])){
          newArray[i] = array[i]
           ...
           newArray[i] = serial(array[i])
       }
    function serial(array){
        var resultado = 0;
    	while (array > 0){
            resultado += array % 10;
            array =  Math.floor(array / 10);
        }
        if (resultado > 9){
        resultado = serial(resultado)
        }
        return resultado
    }
    }
...
function sortArray( array )
{
  var numbers = []
    ...
    return numbers.sort().concat(letters.sort())
